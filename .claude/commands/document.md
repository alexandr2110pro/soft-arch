...
- Use bold for key terms on first use. Use inline code for identifiers (`functionName`, `@space-architects/package-name`).
...
3. **Check file paths and package names** -- Every `@space-architects/...` alias, every `packages/...` path must exist.
