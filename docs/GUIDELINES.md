...
| npm scope | `@space-architects/` | `@space-architects/util-enum` |
...
- Each package exposes a `@space-architects/source` custom export condition pointing to TypeScript source (used for in-monorepo development)
...
```bash
nx g @space-architects/nx-plugin-std:pkg
```
